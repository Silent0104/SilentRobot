...
from SilentRobot.modules.helper_funcs.extraction import (
    extract_user,
    extract_unt_fedban,
    extract_user_fban,
)
from SilentRobot.modules.helper_funcs.string_handling import markdown_parser

import SilentRobot.modules.sql.feds_sql as sql

from SilentRobot.modules.helper_funcs.alternate import (
    send_message,
    typing_action,
    send_action,
)
from SilentRobot.modules.helper_funcs.decorators import Silentcmd, Silentcallback
...
@typing_action
@Silentcmd(command='delfed', pass_args=True)
...
@typing_action
@Silentcmd(command='chatfed', pass_args=True)
...
@typing_action
@Silentcmd(command='joinfed', pass_args=True)
...
@typing_action
@Silentcmd(command='fpromote', pass_args=True)
...
@typing_action
@Silentcmd(command='fdemote', pass_args=True)
...
@typing_action
@Silentcmd(command='fedinfo', pass_args=True)
...
@typing_action
@Silentcmd(command='fedadmins', pass_args=True)
...
@typing_action
@Silentcmd(command=['fban', 'fedban'], pass_args=True)
def fed_ban(update, context):  # sourcery no-metrics
    ...
    if int(user_id) in (777000, 5278339583):
...
@typing_action
@Silentcmd(command=['unfban', 'rmfedban'], pass_args=True)
...
@typing_action
@Silentcmd(command='frules', pass_args=True)
...
@typing_action
@Silentcmd(command='fbroadcast', pass_args=True)
...
@Silentcmd(command='fbanlist', pass_args=True, pass_chat_data=True)
    ...
    if args:
        if args[0] == "json":
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Silent_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Silent_fbanned_users.csv",
...
@typing_action
@Silentcmd(command='fednotif', pass_args=True)
...
@typing_action
@Silentcmd(command='fedchats', pass_args=True)
...
@typing_action
@Silentcmd(command='importfbans', pass_args=True, pass_chat_data=True)
...
@Silentcallback(pattern=r"rmfed_")
...
@typing_action
@Silentcmd(command='fbanstat', pass_args=True)
...
@typing_action
@Silentcmd(command='setfedlog', pass_args=True)
...
@typing_action
@Silentcmd(command='unsetfedlog', pass_args=True)
...
@typing_action
@Silentcmd('subfed', pass_args=True)
...
@typing_action
@Silentcmd(command='unsubfed', pass_args=True)
...
@typing_action
@Silentcmd(command='fedsubs', pass_args=True)
...
@typing_action
@Silentcmd(command='myfeds', pass_args=True)
...
from SilentRobot.modules.language import gs
...
@Silentcallback(pattern=r"fed_help_")
